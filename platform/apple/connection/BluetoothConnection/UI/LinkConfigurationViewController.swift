...
class LinkConfigurationViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // Toggle edit mode
        editButton.rx.tap
            .subscribe(onNext: { [weak self] in self?.viewModel.edit.accept(true) })
            .disposed(by: disposeBag)

        // Cancel updates
        cancelButton.rx.tap
            .subscribe(onNext: { [weak self] in self?.viewModel.cancel.onNext(()) })
            .disposed(by: disposeBag)

        // Save updates
        doneButton.rx.tap
            .subscribe(onNext: { [weak self] in self?.viewModel.done.onNext(()) })
            .disposed(by: disposeBag)
    }
    ...
    override public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        switch Section(rawValue: section)! {
        case .fastMode: return FastModeRow.allCases.count
        case .slowMode: return SlowModeRow.allCases.count
        case .dle: return DleRow.allCases.count
        case .mtu: return MtuRow.allCases.count
        case .preferredMode: return ModeRow.allCases.count
        }
    }
...
private enum FastModeRow: Int, CaseIterable {
    case min
    case max
    case slaveLatency
...
private enum SlowModeRow: Int, CaseIterable {
    case min
    case max
    case slaveLatency
    case supervisionTimeout
...
private enum DleRow: Int, CaseIterable {
    case maxTxPDU
    case maxTxTime
}

private enum MtuRow: Int, CaseIterable {
    case mtu
}

private enum ModeRow: Int, CaseIterable {
    case mode
}
