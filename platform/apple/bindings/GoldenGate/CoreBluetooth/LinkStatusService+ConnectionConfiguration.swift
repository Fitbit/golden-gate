...
extension LinkStatusService.ConnectionConfiguration: RawRepresentable {
    public init?(rawValue: Data) {
        guard rawValue.count >= 9 else {
            LogBindingsWarning("ConnectionConfiguration was \(rawValue.count) bytes - expected: 9")
            return nil
        }
        ...
        if let mode = Mode(rawValue: rawMode) {
            ...
            LogBindingsWarning("Unknown ConnectionConfiguration.Mode: \(rawMode) (\(String(rawMode, radix: 2)))")
            self.mode = .fast
        }
    }
