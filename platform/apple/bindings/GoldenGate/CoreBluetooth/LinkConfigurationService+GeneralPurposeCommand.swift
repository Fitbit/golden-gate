...
extension LinkConfigurationService.GeneralPurposeCommand: RawRepresentable {
    public init?(rawValue: Data) {
        guard rawValue.count >= 1 else {
            LogBindingsWarning("GeneralPurposeCommand was \(rawValue.count) bytes - expected: >= 1")
            return nil
        }
        ...
        do {
            ...
            if let code = Code(rawValue: rawCode) {
                ...
                LogBindingsWarning("Unknown GeneralPurposeCommand: \(rawCode) (\(String(rawCode, radix: 2)))")
