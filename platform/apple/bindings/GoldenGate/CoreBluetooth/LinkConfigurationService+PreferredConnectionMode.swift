...
extension LinkConfigurationService.PreferredConnectionMode: RawRepresentable {
    public init?(rawValue: Data) {
        guard rawValue.count >= 1 else {
            LogBindingsWarning("PreferredConnectionMode was \(rawValue.count) bytes - expected: 1")
            return nil
        }
        ...
        do {
            ...
            if let mode = Mode(rawValue: rawMode) {
                ...
                LogBindingsWarning("Unknown PreferredConnectionMode: \(rawMode) (\(String(rawMode, radix: 2)))")
                self.mode = .fast
            }
        }
    }
