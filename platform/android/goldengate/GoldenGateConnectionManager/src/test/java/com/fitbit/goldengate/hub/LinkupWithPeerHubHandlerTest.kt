...
class LinkupWithPeerHubHandlerTest {
  ...
  private val mockTransmitCharacteristicSubscriptionListener =
    mock<TransmitCharacteristicSubscriptionListener>()
  ...
  fun `complete the linkup procedure`() {
    ...
    testObserver.assertComplete().dispose()
  }
  ...
  fun `should fail if service discovery failed`() {
    ...
    testObserver.assertFailureAndMessage(RuntimeException::class.java, "Failed").dispose()
  }
  ...
  fun `should fail if Link configuration service is not found`() {
    ...
    testObserver.assertFailure(GattServiceNotFoundException::class.java).dispose()
  }
  ...
  fun `fail to wait for gattlink subscription before timeout`() {
    ...
    testObserver.assertFailure(TimeoutException::class.java).dispose()
  }
  ...
  private fun mockDiscoverServicesSuccess(services: List<BluetoothGattService> = emptyList()) =
    whenever(mockGattServiceDiscoverer.discover(mockGattConnection))
      .thenReturn(Single.just(services))

  private fun mockDiscoverServicesFailure() =
    whenever(mockGattServiceDiscoverer.discover(mockGattConnection))
      .thenReturn(Single.error(RuntimeException("Failed")))

  private fun mockLinkConfigurationServiceNotFound() =
    whenever(
        mockPeerGattServiceSubscriber.subscribe(
          mockCentral,
          LinkConfigurationService.uuid,
          ClientPreferredConnectionConfigurationCharacteristic.uuid
        )
  ...
  private fun mockWaitForGattlinkSubscriptionSuccess() =
    whenever(
        mockTransmitCharacteristicSubscriptionListener.getDataObservable(
  ...
  private fun mockWaitForGattlinkSubscriptionTooLate() =
    whenever(
      ...
      .thenReturn(
        Observable.just(GattCharacteristicSubscriptionStatus.ENABLED)
          .delay(LINK_UP_TIMEOUT_SECONDS, TimeUnit.SECONDS)
      )
