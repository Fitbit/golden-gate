...
class ReceiveCharacteristicDataListenerTest {
  ...
  private val device2 = mock<BluetoothDevice> { on { address } doReturn "2" }
  ...
  private val listener =
    ReceiveCharacteristicDataListener(Schedulers.trampoline(), mockGattServerResponseSenderProvider)
  ...
  fun shouldReceiveDataOnRegisteredReceiver() {
    ...
    dataObservable.test().assertValue { Arrays.equals(it, data) }
  }
  ...
  fun shouldOnlyReceiveDataOnMatchingDeviceReceiver() {
    ...
    dataObservable1.test().assertValue { Arrays.equals(it, data) }

    // verify no data available
    dataObservable2.test().assertNoValues()
  }
  ...
  fun shouldIgnoreIfCallbackIsNotForReceiveCharacteristic() {
    ...
    dataObservable.test().assertNoValues()
  ...
  fun shouldOnlyReceiveDataOnLatestRegisteredDeviceReceiver() {
    ...
    dataObservable1.test().assertNoValues()

    // verify data is only available on latest registered receiver
    dataObservable2.test().assertValue { Arrays.equals(it, data) }
  }
