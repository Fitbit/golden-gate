...
class StackTest : BaseTest() {
    ...
    private val mockNodeKey:NodeKey<String> = mock {
        on { value } doReturn "node_key_1"
    }
    ...
    fun shouldEmitTlsStateChangeToHandshakeOnStackStart() {
        Stack(mockNodeKey,txSink.thisPointer, rxSource.thisPointer).use { stack ->
            ...
            stack.dtlsEventObservable
                    .test()
                    .assertValue { it.state == DtlsProtocolStatus.TlsProtocolState.TLS_STATE_HANDSHAKE }
        }
    }
    ...
    fun shouldEmitTlsStateChangeToSession() {
        Stack(mockNodeKey,txSink.thisPointer, rxSource.thisPointer).use { stack ->
            ...
            stack.dtlsEventObservable
                    .test()
                    .assertValue { it.state == DtlsProtocolStatus.TlsProtocolState.TLS_STATE_SESSION }
        }
    }
    ...
    fun shouldEmitUnknownTlsStateChange() {
        Stack(mockNodeKey,txSink.thisPointer, rxSource.thisPointer).use { stack ->
            ...
            stack.dtlsEventObservable
                    .test()
                    .assertValue { it.state == DtlsProtocolStatus.TlsProtocolState.TLS_STATE_UNKNOWN }
        }
    }
    ...
    fun shouldEmitStackEvent() {
        Stack(mockNodeKey,txSink.thisPointer, rxSource.thisPointer).use { stack ->
            ...
            stack.stackEventObservable
                .test()
                .assertValue { it == StackEvent.GattlinkBufferOverThreshold }
        }
    }
    ...
    fun shouldEmitStackStalledEvent() {
        Stack(mockNodeKey,txSink.thisPointer, rxSource.thisPointer).use { stack ->
            stack.onStackEvent(StackEvent.GattlinkSessionStall(ONE_MINUTE_IN_MILLISECONDS).eventId, ONE_MINUTE_IN_MILLISECONDS)
            stack.stackEventObservable
                .test()
                .assertValue { it is StackEvent.GattlinkSessionStall && it.stalledTime == ONE_MINUTE_IN_MILLISECONDS}
        }
    }
