...
class OutgoingRequestBuilderTest {
  ...
  fun shouldBuildRequestWithBodyAndObserver() {
    ...
    val request =
      OutgoingRequestBuilder("foo", Method.GET)
  ...
  fun shouldBuildRequestWithBodyInputStream() {
    ...
    Assert.assertTrue(
      Arrays.equals(testByteArray, (request.body as InputStreamOutgoingBody).data.readBytes())
    )
  }
  ...
  private fun getPath(options: Options): List<Option> {
    return options.filter { option -> option.number == OptionNumber.URI_PATH }
  }
}
